...
> в обучении мы их **не используем** — обучение идёт против
...
- **Тривиальные ablation-эксперименты.** Если завтра захочется сравнить
  «PPO с жёсткими приводами vs PPO с мягкими» — меняешь *одну строчку*
  (`NATURAL_FREQ = 5 * 2π` вместо `10 * 2π`), и все 29 шарниров
  согласованно перестраиваются. С XML пришлось бы либо писать
...
- **Бедро roll сильно антисимметрично между ногами** (для левой ноги от
...
когда в режиме sim-to-sim получит такой же шум от реального датчика.
...
- **Перекат стопы — непрерывный, а не дискретный процесс.** Когда нога
  ...
  носку. У 4 угловых сфер между передней и задней парой просто **нет
  ничего**: при перекате центр давления (Center of Pressure, CoP) на
  одном кадре резко перебрасывается с одних сфер на другие. Для
  симулятора это ступенька в reaction-force, а для политики —
  скачкообразное возмущение, которое приходится постоянно
  компенсировать. С 7 капсулами вдоль подошвы контакт «течёт» плавно.
...
<p align="center">
  <img src="assets/g1/g1_mjx_colliders.png" alt="MJX-коллайдеры робота G1" width="42%">
</p>
<p align="center">
  <em>Рисунок 3. Коллайдеры MJX-варианта: вместо точных мешей всего тела
  используется набор простых примитивов — капсул и сфер, специально
  подобранных вручную. Это даёт на порядок более быструю и численно
  стабильную работу решателя контактов на GPU при практически той же
  физической достоверности для задач локомоции.</em>
</p>
